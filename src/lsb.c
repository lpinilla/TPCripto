...
payload extract_payload(lsb l, carrier c) {
  ...
  if (p->size >= c->c_size) {
    printf("Cannot extract file, it is too big\n");
    return NULL;
  }
...
payload extract_payload_lsbi(carrier c) {
  ...
  // ya tengo el contenido encriptado con RC4 en p->content, y el size
  // desencriptado en p->size, y el size encriptado en payload_size_enc y el
  // size encriptado en 4 bytes en prep_size

  // en plaintext vamos a guardar el payload desencriptado mas el tamanio al
  // principio el tamanio es 4(bytes para el tamanio)+tamanio del
  // archivo(p->size)+5(extencion)
