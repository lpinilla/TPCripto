...
{
  char *str = ".txt";
  uint8_t *ext = analice_name("test2.txt", NULL);
  if (strcmp(str, (char *)ext) != 0)
  {
    assert_true(1 == 0);
  }
...
void hidden_file_concat()
{
  ...
  printf("CONCAT: %d%d%d%d", concat[0], concat[1], concat[2], concat[3]);
  printf("%s\n", concat + 4);
  assert_true(1 == 1);
}
